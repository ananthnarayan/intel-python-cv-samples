...
def calibrate_gauge(gauge_number, file_type):
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, 20, np.array([]), 100, 50, int(height*0.2), int(height*0.8))
    print(circles)
    # average found circles, found it to be more accurate than trying to tune HoughCircles parameters to get just the right one
    a, b, c = circles.shape # num rows, num columns, num values in each tuple.
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type):
    ...
    #diff2LowerBound = 0.50 #diff2LowerBound and diff2UpperBound determine how close the other point of the line should be to the outside of the gauge
    #diff2UpperBound = 1.0
    
    #run through a bunch of values to see if we can get atleast one line. 
    #when we get one, we break the loop. The zero index values are defaults
    #provided with the code. 
    diff1LowerBoundArray = [0.15, 0.10]
    diff1UpperBoundArray = [0.25, 0.30]
    
    diff2LowerBoundArray = [0.50, 0.45]
    diff2UpperBoundArray = [1.00, 1.00]
    
    for diffIndex in range(0, len(diff1LowerBoundArray)):
        diff1LowerBound = diff1LowerBoundArray[diffIndex]
        diff1UpperBound = diff1UpperBoundArray[diffIndex]
        
        diff2LowerBound = diff2LowerBoundArray[diffIndex]
        diff2UpperBound = diff2UpperBoundArray[diffIndex]
        ...
        for i in range(0, len(lines)):
            ...
            if len(final_line_list) > 0:
                print(("Detected lines at index: %d ") % (diffIndex))
                break
    ...
    #    cv2.line(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
    if len(final_line_list) == 0:
        print("Unable to determine line")
        return 0
